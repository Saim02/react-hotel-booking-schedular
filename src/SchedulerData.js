...
export default class SchedulerData {
  constructor(
    ...
    this.events = [];
    this.eventGroups = [];
    this.roomClassPriceWithDayList = [];
      ...
      if (
        ...
        if (this.viewType < viewType) {
          if (viewType === ViewTypes.Week) {
            ...
            // this.startDate = this.localeMoment(
  ...
  getContentCellWidth() {
    ...
    return this.isContentViewResponsive()
      ? parseInt(
          (schedulerWidth * Number(contentCellConfigWidth.slice(0, -1))) / 110
        )
  ...
  _resolveDate(num, date = undefined) {
    ...
    if (this.viewType === ViewTypes.Week) {
      ...
      this.startDate =
        date != undefined
          ? this.localeMoment(date).subtract(2, "days").format(DATE_FORMAT)
          : this.localeMoment(this.startDate)
              .add(num, "months")
              .format(DATE_FORMAT);
      this.endDate = this.localeMoment(this.startDate)
      ...
      if (this.cellUnit === CellUnits.Hour) {
        ...
        while (header >= start && header <= end) {
          ...
          if (
            ...
            // Added After
            // let lastIndexEnd = time.lastIndexOf(" ");

            // let modifiedTime = time.substring(0, lastIndexEnd);

            // modifiedTime = modifiedTime + " 12:00:00";
  ...
  _createInitRenderData(isEventPerspective, eventGroups, resources, headers) {
    ...
    slots.forEach((slot) => {
      ...
      let slotRenderData = {
        ...
        rowMaxCount: 0,
        rowHeight:
          this.config.nonAgendaSlotMinHeight !== 0 && slot.groupOnly
            ? this.config.nonAgendaSlotMinHeight
            : this.config.eventItemLineHeight + 20,
        headerItems: headerEvents,
        indent: 0,
        hasChildren: false,
        expanded: true,
        render: true,
        standardPrice: slot.standardPrice ? slot.standardPrice : undefined,
      };
      let id = slot.id;
      let value = undefined;
      if (slotMap.has(id)) {
  ...
  _changeRoomClassName(slotId, newRoomName) {
    this.resources.forEach((resource, index) => {
      if (resource.id === slotId) {
        if (resource.name !== newRoomName) {
          resource.name = newRoomName;
          this._createRenderData();
        }
      }
    });
  ...
  _changeRoomParent(slotId, roomClassSelectedOption) {
    this.resources.forEach((resource, index) => {
      if (resource.id === slotId) {
        resource.parentId = roomClassSelectedOption;
        this._createRenderData();
      }
    });
  }

  _addRoomClassPriceWithDayList(roomType, headerTime, standardPrice) {
    let hasValue = false;

    let lastIndexEnd = headerTime.lastIndexOf(" ");

    let modifiedHeaderTime = headerTime.substring(0, lastIndexEnd);

    modifiedHeaderTime = modifiedHeaderTime + " 12:01:01";

    this.roomClassPriceWithDayList.forEach((element) => {
      if (element.id === roomType + modifiedHeaderTime) {
        hasValue = true;
      }
    });
    if (!hasValue) {
      this.roomClassPriceWithDayList.push({
        id: roomType + modifiedHeaderTime,
        roomType: roomType,
        headerTime: modifiedHeaderTime,
  ...
  _setTableCellPrice(roomType, headerTime, newPrice) {
    let lastIndexEnd = headerTime.lastIndexOf(" ");

    let modifiedHeaderTime = headerTime.substring(0, lastIndexEnd);

    modifiedHeaderTime = modifiedHeaderTime + " 12:01:01";
    this.roomClassPriceWithDayList.forEach((item) => {
      if (item.id === roomType + modifiedHeaderTime) {
        item.standardPrice = parseInt(newPrice, 10);
      }
    });
  }

  _changeRoomClassPriceWithDayListPrice(
    roomType,
    headerTime,
    standardPrice,
    newPrice
  ) {
    let idToCheck = roomType + headerTime + standardPrice;
    // let newPrice = "";
    let length = this.roomClassPriceWithDayList.length;
    for (let i = 0; i < length; i++) {
      if (this.roomClassPriceWithDayList[i].id === idToCheck) {
  ...
  _setNewEventPrice(passedEvent, newPrice) {
    this.events.forEach((event) => {
      if (event.id === passedEvent.id) {
        event.totalPrice = newPrice + event.totalPrice;

        // If previous amount is paid and new price added
        event.amountDue = event.amountDue + newPrice;
  ...
  _setNewEventDueAmount(passedEvent, dueAmount) {
    this.events.map((event) => {
      if (event.id === passedEvent.id) event.amountDue = dueAmount;
    });
  }
  _setNewEventTotalPrice(passedEvent, newTotal) {
    this.events.map((event) => {
      if (event.id === passedEvent.id) event.totalPrice = newTotal;
    });
  }

  _getRoomClassPriceWithDayList(slotId, start, end) {
    let parentId = "";

    let totalPrice = 0;
    let oneDayTripPrice = 0;

    this.resources.forEach((slot, index) => {
      if (slot.id === slotId) {
        parentId = slot.parentId;
      }
    });

    let priceAgainstDayStartIndex = 0;
    let priceAgainstDayEndIndex = 0;

    this.roomClassPriceWithDayList.forEach((priceAgainstDay, index) => {
      if (priceAgainstDay.id === parentId + start) {
        // getting start
        priceAgainstDayStartIndex = index;
      }
      // getting end index
      if (priceAgainstDay.id === parentId + end) {
        priceAgainstDayEndIndex = index;
        // console.warn("End index: " + index);
      }
    });

    // sum Price
    for (let i = priceAgainstDayStartIndex; i < priceAgainstDayEndIndex; i++) {
      totalPrice = totalPrice + this.roomClassPriceWithDayList[i].standardPrice;
    }

    return oneDayTripPrice ? oneDayTripPrice : totalPrice;
  }

  // Confirming reservation from curtosy hold or block

  _confirmreservation(passedEvent) {
    this.events.forEach((event) => {
      if (event.id === passedEvent.id) {
  ...
  _addNoteToEvent(passedEvent, note, date) {
    this.events.forEach((event) => {
      if (event.id === passedEvent.id) {
        event.notes.push({
          text: note,
  ...
  _checkOutGuest(passedEvent) {
    this.events.forEach((event) => {
      if (event.id === passedEvent.id) {
        event.type = 7;
      }
    });
    this.removeEvent(passedEvent);
  }
  _chargeRemaining(passedEvent) {
    this.events.forEach((event) => {
      if (event.id === passedEvent.id) {
        event.amountDue = 0;
        event.paid = true;
  ...
  _changeGuestInfo(passedEvent, valueType, value) {
    this.events.forEach((event) => {
      if (event.id === passedEvent.id) {
        if (valueType === "name") {
          event.title = value;
  ...
  _changeRoomStatus(passedEvent, newStatus) {
    this.events.forEach((event) => {
      if (event.id === passedEvent.id) {
        event.type = newStatus;
      }
    });
  }

  _unAssignRoom(passedEvent) {
    let parentId = 0;

    // this.resources.forEach((resource) => {
    //   if (resource.id === passedEvent.resourceId) parentId = resource.parentId;
    // });

    // this.roomClassPriceWithDayList.forEach((item) => {
    //   if (item.id === parentId + passedEvent.start) {
    //     item.unAssignedEvents.push(passedEvent);
    //   }
    // });
    this.headers.forEach((header) => {
      let lastIndexEnd = header.time.lastIndexOf(" ");

      let modifiedHeaderTime = header.time.substring(0, lastIndexEnd);

      modifiedHeaderTime = modifiedHeaderTime + " 12:01:01";

      if (modifiedHeaderTime === passedEvent.start)
        header.unAssignedEvents.push(passedEvent);
    });
    this.removeEvent(passedEvent);
  }

  _reAssignEvents(unAssignedEventsList, schedulerData, headerRef) {
    // Check Conflict First

    let headerIndex = null;

    this.headers.forEach((header, index) => {
      console.warn("triggerres");
      if (header.time === headerRef.time) headerIndex = index;
    });

    let unAssignedEventsListCopy = unAssignedEventsList;

    unAssignedEventsList.forEach((unAssignedEvent, index) => {
      let hasConflict = false;
      let startTime = unAssignedEvent.start;
      let endTime = unAssignedEvent.end;

      let start = this.localeMoment(startTime),
        end = this.localeMoment(endTime);

      this.events.forEach((e) => {
        console.warn("triggerre ends");
        if (this._getEventSlotId(e) === unAssignedEvent.resourceId) {
          let eStart = this.localeMoment(e.start),
            eEnd = this.localeMoment(e.end);
          if (
            ...
            hasConflict = true;
          alert("Rooms are already occupied");
          console.warn("conflict In ReAssignig");
        }
      });
      if (!hasConflict) {
        this.addEvent(unAssignedEvent);
        unAssignedEventsListCopy.splice(index, 1);
      }
    });

    this.headers[headerIndex].unAssignedEvents = unAssignedEventsListCopy;
    console.warn(this.headers[headerIndex].unAssignedEvents);
  }

  _getParentId(slotId) {
    let parentId = undefined;
    this.resources.forEach((resource) => {
      if (resource.id === slotId) parentId = resource.parentId;
    });
    return parentId;
  }

  _checkConflict(selectedRoomId, startTime, endTime) {
    let hasConflict = false;

    let start = moment(startTime),
      end = moment(endTime);
    this.events.forEach((e) => {
      if (this._getEventSlotId(e) === selectedRoomId) {
  ...
  _getRoomTypesList() {
    ...
    this.resources.forEach((resource) => {
      if (resource.groupOnly && resource.id !== "AllRooms") {
        list.push(resource);
      }
    });
  ...
  _makeReservationMultiType(passedEvent, type, attachingReservationRef = "") {
    this.events.forEach((event) => {
      if (event.id === passedEvent.id) {
        event.groupedReservation = true;
        event.groupedReservationType = type;
        if (type === "primary") {
          if (event.roomsAttached) {
            // alert("adding More");
            event.roomsAttached.push(attachingReservationRef);
          } else {
            // alert("First Time");
            let arrayToAttach = [];
            arrayToAttach.push(attachingReservationRef);
            event.roomsAttached = arrayToAttach;
          }
        } else {
          event.primaryReservationRef = attachingReservationRef;
        }
      }
    });
  }

  _getGrandTotalForGroupedReservation(passedEventId) {
    let grandTotal = 0;
    this.events.forEach((event) => {
      if (event.id === passedEventId) {
        grandTotal = event.totalPrice;
      }
    });
